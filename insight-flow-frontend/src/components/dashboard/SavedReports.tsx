...
export default function SavedReports({ currentFilters, currentMetrics, onLoadReport }: Props) {
  ...
  return (
    <div className="glass-card rounded-2xl border border-[#ddd1f5] bg-white p-6 shadow-[0_12px_30px_rgba(38,25,64,0.1)]">
      <h3 className="mb-4 text-lg font-semibold text-[#2f2450]">Saved Reports</h3>
      ...
      <div className="mb-4 flex gap-2">
        <input
          ...
          className="flex-1 rounded-xl border border-[#ddd1f5] bg-white px-3 py-2 text-sm outline-none focus:border-[#8b5cf6]"
        />
      ...
      {sortedReports.length === 0 ? (
        <p className="py-4 text-center text-sm text-[#7c6f99]">No saved reports yet</p>
      ) : (
        <div className="max-h-64 space-y-2 overflow-y-auto">
          {sortedReports.map((report) => (
            <div
              key={report.id}
              className="flex items-center justify-between rounded-lg border border-[#ddd1f5] bg-[#f7f2ff] p-3"
            >
              <button className="flex-1 text-left" onClick={() => onLoadReport(report.filters)}>
                <p className="text-sm font-medium text-[#3d3160]">{report.title}</p>
                <p className="text-xs text-[#7c6f99]">{new Date(report.createdAt).toLocaleDateString()}</p>
              </button>
              <Button
                variant="outline"
                size="sm"
                className="border-[#d9cdf2] text-[#7c6f99]"
